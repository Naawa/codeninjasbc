import { birthdaySchema, tourSchema } from '$lib/components/forms/validation';
...
export const actions = {
    default: async ({ request, fetch, cookies }) => {
        const form = await superValidate(request, zod(birthdaySchema));
        
        if (!form.valid) {
            return fail(400, { form });
        }

        let utmCampaign = cookies.get("utmCampaign") || ""
        let utmMedium = cookies.get("utmMedium") || ""
        let utmSource = cookies.get("utmSource") || ""
            ...
            .insert([
                {
                    ...
                    tour_date: form.data.desiredPartyDate,
